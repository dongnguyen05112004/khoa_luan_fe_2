        ...
        <div class="col-md-6">
          <div class="info-card-plain h-100">
            ...
            <div v-else class="empty-service">
              ...
              <button class="btn-go-buy" @click="activeTab = 'marketplace'; activePlanType = 'pt'">Tìm HLV ngay</button>
            </div>
          </div>
        </div>
      </div>
...
.mk-btn-register.disabled, .mk-btn-detail.disabled, .btn-book-confirm.disabled {
...
.btn-book-confirm:hover:not(:disabled) {
  transform: translateY(-1px);
  box-shadow: 0 6px 16px rgba(22,163,74,.35);
  opacity: .95;
}
.btn-book-confirm:active:not(:disabled) {
...
.btn-apply-promo:hover:not(:disabled) {
  background: #166534;
  box-shadow: 0 4px 10px rgba(15,76,42,.2);
}
