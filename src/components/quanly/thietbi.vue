          ...
          <div class="panel-section">
            ...
            <div class="spec-grid">
              <div class="spec-item">
                ...
                <div class="spec-value">{{ selectedDevice.brand || '—' }}</div>
              </div>
              <div class="spec-item">
                <div class="spec-label">Loại</div>
                <div class="spec-value">{{ selectedDevice.type || '—' }}</div>
              </div>
              <div class="spec-item">
                <div class="spec-label">Khu vực</div>
                <div class="spec-value">{{ selectedDevice.location || (selectedDevice.branch &&
                  selectedDevice.branch.name) || '—' }}</div>
              </div>
              <div class="spec-item">
                <div class="spec-label">Năm mua</div>
                <div class="spec-value">{{ selectedDevice.purchase_date ? selectedDevice.purchase_date.substring(0, 4) :
                  '—' }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="panel-section">
            ...
            <div v-else class="maint-list">
              <div class="maint-item" v-for="m in deviceMaintenances" :key="m.id">
                ...
                <button class="btn-del-maint" @click="deleteMaintenance(m.id)" title="Xóa lịch"><i
          ...
          <div class="modal-body">
            <div class="form-row">
              <div class="form-group">
                ...
                <input v-model="deviceForm.equipment_name" type="text" placeholder="VD: Máy chạy bộ NordicTrack"
                  class="form-input" />
              </div>
              <div class="form-group">
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input v-model="deviceForm.brand" type="text" placeholder="VD: Matrix, LifeFitness"
                  class="form-input" />
              </div>
              <div class="form-group">
                <label>Ngày mua</label>
                <input v-model="deviceForm.purchase_date" type="date" class="form-input" :max="today" />
              </div>
            </div>
          ...
          <div class="modal-body">
            <div class="form-group full">
              ...
              <select v-model="maintForm.equipment_id" class="form-input">
                ...
                <option v-for="d in devices" :key="d.id" :value="d.id">{{ d.equipment_name }} ({{ d.serial_number ||
                  d.id }})</option>
              </select>
            </div>
            <div class="form-row">
            ...
            <div class="form-group full">
              <label>Ghi chú</label>
              <textarea v-model="maintForm.notes" rows="3" placeholder="Mô tả công việc bảo trì..."
        ...
        <div class="modal-box modal-wide">
          <div class="modal-header">
            ...
            <div style="display:flex;gap:8px;align-items:center;">
              <button class="btn-add-inline" @click="openAddMaintenanceModal(null)"><i class="fas fa-plus"></i> Thêm
                lịch</button>
              <button class="modal-close" @click="showMaintenanceListModal = false"><i
                  class="fas fa-times"></i></button>
            </div>
          </div>
            ...
            <table v-else class="maint-table">
              ...
              <tbody>
                <tr v-for="m in allMaintenances" :key="m.id">
                  ...
                  <td><button class="btn-del-maint" @click="deleteMaintenance(m.id)" title="Xóa"><i
...
.kpi-card.accent-green .kpi-icon-wrap {
  background: #dcfce7;
  color: #16a34a;
}

.kpi-card.accent-blue .kpi-icon-wrap {
  background: #dbeafe;
  color: #2563eb;
}

.kpi-card.accent-orange .kpi-icon-wrap {
  background: #fff3e0;
  color: #d97706;
}

.kpi-card.accent-red .kpi-icon-wrap {
  background: #fee2e2;
  color: #dc2626;
}
...
.kpi-card.accent-green .kpi-border-accent {
  background: #16a34a;
}

.kpi-card.accent-blue .kpi-border-accent {
  background: #2563eb;
}

.kpi-card.accent-orange .kpi-border-accent {
  background: #d97706;
}

.kpi-card.accent-red .kpi-border-accent {
  background: #dc2626;
}
